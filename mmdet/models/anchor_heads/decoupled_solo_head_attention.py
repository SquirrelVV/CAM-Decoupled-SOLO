...
from torch.nn import Parameter, Softmax, Dropout,BatchNorm2d, InstanceNorm2d
...
from .solo_attention import CAM_Module
...
class DecoupledSOLOHeadAttention(nn.Module):
    def __init__(self,
                 num_classes,
                 in_channels,
                 ...
                 conv_cfg=None,
                 norm_cfg=None):
        super(DecoupledSOLOHeadAttention, self).__init__()
        ...
        self.channels_attention = CAM_Module(self.seg_feat_channels)
        self.conv51 = nn.Sequential(nn.Conv2d(self.seg_feat_channels, self.seg_feat_channels, 3, padding=1, bias=False),
                                    InstanceNorm2d(self.seg_feat_channels),
                                    nn.ReLU())

        self.conv6 = nn.Sequential(nn.Dropout2d(0.1, False), nn.Conv2d(self.seg_feat_channels, self.seg_feat_channels, 1))
        self.conv_x = nn.Sequential(nn.Conv2d(self.seg_feat_channels * 2, self.seg_feat_channels, 3, padding=1, bias=False),
                                    InstanceNorm2d(self.seg_feat_channels),
                                    nn.ReLU())
        self.conv_y = nn.Sequential(nn.Conv2d(self.seg_feat_channels * 2, self.seg_feat_channels, 3, padding=1, bias=False),
                                    InstanceNorm2d(self.seg_feat_channels),
                                    nn.ReLU())
        self._init_layers()
    ...
    def init_weights(self):
        ...
        for m in self.conv6:
            if isinstance(m, nn.Conv2d):
                ...
                if m.bias is not None:
                    nn.init.constant_(m.bias, 0)
        for m in self.conv_x:
            if isinstance(m, nn.Conv2d):
                nn.init.normal_(m.weight, std=0.01)
                if m.bias is not None:
                    nn.init.constant_(m.bias, 0)
        for m in self.conv_y:
            if isinstance(m, nn.Conv2d):
                nn.init.normal_(m.weight, std=0.01)
                if m.bias is not None:
                    nn.init.constant_(m.bias, 0)
    ...
    def forward_single(self, x, idx, eval=False, upsampled_size=None):
        ...
        # feature attention
        feat_c = self.channels_attention(ins_feat)
        sc_conv1 = self.conv51(feat_c)
        sc_conv = F.interpolate(self.conv6(sc_conv1), scale_factor=2, mode='bilinear')
        ins_feat_x = torch.cat((sc_conv, ins_feat_x), 1)
        ins_feat_y = torch.cat((sc_conv, ins_feat_y), 1)

        ins_feat_x = self.conv_x(ins_feat_x)
        ins_feat_y = self.conv_y(ins_feat_y)
